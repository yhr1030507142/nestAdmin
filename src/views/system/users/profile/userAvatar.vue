<template>
  <div>
    ...
    <el-dialog :title="title" v-model="open" width="1000px" append-to-body @opened="modalOpened">
      <el-row>
      ...
      <el-row>
        <el-col :lg="2" :md="2">
          <el-upload action="#" :http-request="requestUpload" :show-file-list="false" :before-upload="beforeUpload">
            <el-button>
              上传
              <!-- <el-icon-upload class="el-icon-upload el-icon--right"></el-icon-upload> -->
            </el-button>
          </el-upload>
        </el-col>
  ...
  methods: {
    ...
    beforeUpload(file) {
      if (file.type.indexOf('image/') == -1) {
        $sdk.msgError('文件格式错误，请上传图片类型,如：JPG，PNG后缀的文件。')
      } else {
    ...
    uploadImg() {
      this.$refs.cropper.getCropBlob((file) => {
        ...
        uploadAvatar(formData).then(({ data }) => {
          this.open = false
          this.options.img = data.url
          useUserStore().avatar = data.url
          $sdk.msgSuccess('修改成功')
          this.visible = false
        })
      })
...
.img-lg {
  width: 120px;
  height: 120px;
  border: 1px solid var(--BorderBlack8);
}
